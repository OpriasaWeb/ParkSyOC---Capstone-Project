...
$stmt = $pdo->query("SELECT * FROM messages WHERE mssg_confirmation = 'Confirmed' ");
$stmt->execute();
...
<body>
  ...
  <div class="jumbutron-section mb-5">
    <div class="mt-3 p-3 bg-success text-white rounded">
      ...
      <p>Message queries / Read queries</p>
    </div>
  </div>
    ...
    <table id="datatableid" class="table table-hover border-secondary">
      <thead>
        <tr class="text-center">
          ...
          <th scope="col-3">Message queries</th>
      ...
      <tbody>
          <tr>
            ...
            <td><?php echo $row['mssg_queries'] ?></td>
            <td>
              <?php 
                if($row['mssg_confirmation'] == 'Confirmed'){
            ...
            <td>
              <form action="" method="POST">
                ...
                <a href="./queriesCnfrmdDelete.php?<?php echo "deleteId=" . $row['message_id'] ?>" class="btn btn-sm btn-outline-danger" name="delete" value="">Delete</a>
              </form>
            </td>
          </tr>
          <?php endforeach; ?>
